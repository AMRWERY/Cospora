      ...
      <table class="w-full table-auto text-start min-w-max">
        ...
        <tbody>
          <tr class="border-b hover:bg-slate-50 border-slate-200"
            ...
            <td class="p-4 py-5">
              <p class="block text-sm text-slate-500">{{ (checkoutStore.currentPage - 1) * checkoutStore.ordersPerPage +
                index +
                1 }}</p>
            </td>
            <td class="p-4 py-5">
            ...
            <td class="p-4 py-5">
              <div class="flex space-s-2">
                <button @click="updateOrderStatus(order.id, 'Accepted')" :disabled="order.status === 'Accepted'">
                  <icon v-if="order.loading && order.action === 'Accepted'" name="svg-spinners:6-dots-rotate"
                    size="20px" class="text-green-500" />
                  <icon name="material-symbols:check-circle"
                    class="w-8 h-8 text-white bg-green-500 rounded hover:bg-green-600 disabled:opacity-50" v-else />
                </button>
                <button @click="updateOrderStatus(order.id, 'Rejected')" :disabled="order.status === 'Rejected'">
                  <icon v-if="order.loading && order.action === 'Rejected'" name="svg-spinners:6-dots-rotate"
                    size="20px" class="text-red-500" />
                  <icon name="material-symbols:cancel"
                    class="w-8 h-8 text-white bg-red-500 rounded hover:bg-red-600 disabled:opacity-50" v-else />
                </button>
              </div>
            </td>
          </tr>
